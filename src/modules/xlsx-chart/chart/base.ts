  ...
  */
  writeMultTable: function (row, cb) {
    ...
    me.read({ file: 'xl/worksheets/sheet2.xml' }, function (err, o) {
      ...
      o.worksheet.dimension.$.ref = `A${row}:${me.getColName(
        me.titles.length + 1,
  ...
  */
  removeUnusedCharts: function (o) {
    ...
    _.each(CHART_TYPES, function (chart) {
      ...
      if (
        o['c:chartSpace']['c:chart']['c:plotArea']['c:' + chart + 'Chart']
          .length
      ) {
        _.each(
          o['c:chartSpace']['c:chart']['c:plotArea']['c:' + chart + 'Chart'],
    ...
    me.read({ file: 'xl/charts/chart1.xml' }, function (err, o) {
      ...
      _.each(me.titles, function (t, i) {
        ...
        var grouping =
          me.data[t].grouping ||
          me.grouping ||
          CHART_GROUPING_BY_CHART_NAME[chart]
          ...
          if (customColors.points) {
            customColorsPoints['c:dPt'] = chartOpts.fields
              .map(function (field, i) {
                const color = _.chain(customColors)
                  .get('points')
                  .get(t)
                  .get(field, null)
                  .value()
        ...
        var ser = {
          ...
          'c:tx': {
            'c:strRef': {
              'c:f': "'Chart Data'!$" + me.getColName(i + 2) + '$' + row,
              'c:strCache': {
                'c:ptCount': {
                  $: {
                    val: 1,
      ...
      const templateChartName =
        ...
        CHART_TYPES.find(
          chartType =>
            me.chartTemplate['c:chartSpace']['c:chart']['c:plotArea'][
              `c:${chartType}Chart`
            ],
        ) +
        'Chart'
      ...
      _.each(seriesByChart, function (ser, chart) {
        ...
        if (!chartTagName) {
          return cb(
            new VError(
              new Error(`Chart type '${chart}' is not supported`),
              'writeChart',
            ),
          )
        }
      ...
      me.removeUnusedCharts(o)
      me.changeLineChartToSecondaryAxis(o)

      if (me.chartTitle) {
        me.writeTitle(o, me.chartTitle, chartOpts)
      }
      me.write({ file: `xl/charts/chart${chartN + 1}.xml`, object: o })
      cb()
    })
  },

  changeLineChartToSecondaryAxis: function (o) {
    var axIdList = [
      {
        $: {
          val: '149222784',
    ...
    if (o['c:chartSpace']['c:chart']['c:plotArea']['c:lineChart']) {
      o['c:chartSpace']['c:chart']['c:plotArea']['c:barChart'][0]['c:axId'] =
        axIdList
      const catAx = {
        ...o['c:chartSpace']['c:chart']['c:plotArea']['c:catAx'][0],
      }
      catAx['c:axId'] = axIdList[0]
      catAx['c:crossAx'] = axIdList[1]
      catAx['c:delete'] = {
        $: {
          val: '1',
        },
      ...
      const valAx = {
        ...o['c:chartSpace']['c:chart']['c:plotArea']['c:valAx'][0],
      }
      valAx['c:axId'] = axIdList[1]
      valAx['c:axPos'] = {
        $: {
          val: 'r',
        },
      }
      valAx['c:crossAx'] = axIdList[0]
      valAx['c:crosses'] = {
        $: {
          val: 'max',
        },
      }
      o['c:chartSpace']['c:chart']['c:plotArea']['c:valAx'][0]['c:tickLblPos'] =
        {
          $: {
            val: 'high',
          },
      ...
      o['c:chartSpace']['c:chart']['c:plotArea']['c:catAx'][1] = catAx
      o['c:chartSpace']['c:chart']['c:plotArea']['c:valAx'][1] = valAx
    }
  },
  /*
    Chart title
    ...
    async.series(
      [
        function (cb) {
          ...
          let path = envConfig.excelTemplateFolderPath + me.tplName + '.xlsx'
    ...
    async.series(
      [
        function (cb) {
          ...
          let path = envConfig.excelTemplateFolderPath + 'mult.xlsx'
        ...
        function (cb) {
          me.titles = []
          me.fields = []
          me.charts.push(me.charts[me.charts.length - 1])
          ...
          async.eachOfSeries(
            ...
            (chart, key, cb) => {
              ...
              const deleteEmptyCells =
                me.deleteEmptyCells || chart.deleteEmptyCells

              _.each(me.titles, function (t) {
                me.data[t] = me.data[t] || {}

                _.each(me.fields, function (f) {
                  const value = normalizeValue(me.data[t][f], {
                    deleteEmptyCells,
                  })
                  me.data[t][f] = value
                })
              })
              if (key < me.charts.length - 1) {
                  ...
                  function (cb) {
                    me.read(
                      ...
                      function (err, o) {
                        ...
                        if (n == 1) {
                          o['xdr:wsDr']['xdr:twoCellAnchor']['xdr:from'][
                            'xdr:col'
                          ] = position.fromColumn
                          o['xdr:wsDr']['xdr:twoCellAnchor']['xdr:from'][
                            'xdr:colOff'
                          ...
                          o['xdr:wsDr']['xdr:twoCellAnchor']['xdr:to'][
                            'xdr:col'
                          ] = position.toColumn
                          o['xdr:wsDr']['xdr:twoCellAnchor']['xdr:to'][
                            'xdr:colOff'
