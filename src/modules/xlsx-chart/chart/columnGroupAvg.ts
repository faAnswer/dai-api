...
var Chart = BaseChart.extend({
  ...
  writeTable: function (cb) {
    ...
    me.read({ file: 'xl/worksheets/sheet2.xml' }, function (err, o) {
      ...
      o.worksheet.dimension.$.ref =
        'A1:' + me.getColName(me.fields.length + 1) + '3'
      var rows = []
      // groups
      var cells = [],
        col = 2,
  ...
  writeChart: function (cb) {
    ...
    me.read({ file: 'xl/charts/chart1.xml' }, function (err, o) {
      ...
      var ser = [
        {
          ...
          'c:cat': {
            'c:multiLvlStrRef': {
              'c:f':
                'Table!$B$1:$' + me.getColName(me.fields.length + 1) + '$2',
              'c:multiLvlStrCache': {
                'c:ptCount': {
                  $: {
          ...
          'c:val': {
            'c:numRef': {
              'c:f':
                'Table!$B$3:$' + me.getColName(me.fields.length + 1) + '$3',
              'c:numCache': {
      ...
      if (me.showVal) {
        o['c:chartSpace']['c:chart']['c:plotArea']['c:barChart']['c:dLbls'][
          'c:showVal'
        ] = {
          $: {
            val: '1',
      ...
      var ser = [
        {
          ...
          'c:val': {
            'c:numRef': {
              'c:f':
                'Table!$B$4:$' + me.getColName(me.fields.length + 1) + '$4',
              'c:numCache': {
                'c:formatCode': 'General',
                'c:ptCount': {
                  $: {
    ...
    async.series(
      [
        function (cb) {
          ...
          fs.readFile('data/excelTemplate//columnGroupAvg.xlsx', function (
            err,
            data,
          ) {
            if (err) {
